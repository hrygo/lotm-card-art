...
感谢你帮助改进这个卡牌制作脚手架。这里的首要目标是保持事实诚实、证据可追溯、卡面语义可回读，以及让后续贡献者能复现检查结果。
...
仓库目前是 Python 3.10+、标准库、无第三方依赖的制作脚手架。不要为了方便引入需要联网、密钥或付费服务才能运行的步骤。
...
脚手架检查应报告 220 个卡位且没有结构错误；当前测试套件应报告 48 项测试、0 个失败。design 和 release 在没有完成研究、实际图像和真实审核时失败是预期行为。
