...
> **2026-09-18 更名**：本仓库内客户端的应用显示名为「**卡牌演示**」（bundle id `com.hrygo.lotm-card-art`，打包产物 `.build/卡牌演示.app`）；包、模块与源码目录仍是 ASCII `WorldOfMysteries`。本次只更名与换标识，不裁剪功能。见 `docs/DECISIONS.md` D26。
