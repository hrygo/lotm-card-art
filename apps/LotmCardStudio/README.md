...
- S00「愚者」已接入用户批准的 6 条克莱恩叙事文案、`uncle_fu` 系统音色和 6 个本地 WAV；奥黛丽「正义」S07「心理医生」身份卡已接入用户批准的 6 条文案、`serena` 系统音色和 6 个本地 WAV；序列9克莱恩卡已接入 6 条可阅读但待逐字确认的叙事文案，尚未生成声音；批准文本优先播放本地资源，缺失时才回退到 SpeechRail 合成；
- macOS 26-only 界面基线：`NavigationSplitView`、系统 toolbar/search，以及卡片、面板和操作控件的 Liquid Glass 交互材质；
- debug / release `.app` 打包脚本和本机 QA 记录。

当前使用 4 张隔离 fixture 卡：两张 `klein` 角色身份卡、一张 `愚者·序列 0` 候选卡和一张 `audrey` 的「正义·序列 7·心理医生」身份卡。序列9克莱恩卡绑定此前制作的整卡图，卡图随 `.app` 打包；其 6 条新增叙事内容可阅读但仍待逐字确认。S00 与奥黛丽卡的叙事文案及本地试听音频已完成批准链路。
