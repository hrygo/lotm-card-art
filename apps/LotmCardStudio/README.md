...
- S00「愚者先生」和 S09「克莱恩·莫雷蒂」已绑定当前生产源的原生候选卡图；两张卡各有 6 条新版叙事文案与本地 WAV，S00 使用 `uncle_fu`、S09 使用实际可用的 `dylan`，批准文本优先播放本地资源，缺失时才回退到 SpeechRail 合成；
- macOS 26-only 界面基线：`NavigationSplitView`、系统 toolbar/search，以及卡片、面板和操作控件的 Liquid Glass 交互材质；
- debug / release `.app` 打包脚本和本机 QA 记录。

当前只使用 2 张隔离 fixture 卡：S09「克莱恩·莫雷蒂」和 S00「愚者先生」。两张卡均绑定当前生产源的 1024×1536 原生候选卡图、六维回读、6 条新版叙事和 6 个本地 WAV；视觉候选仍需用户单独复核，不自动计入正式卡牌。正义小姐不再进入 App fixture，但其 App 外源资产继续独立保留。
