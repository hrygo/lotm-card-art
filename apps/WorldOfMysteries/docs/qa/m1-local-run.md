...
## 2026-09-15 改名追加：客户端更名 WorldOfMysteries 与数据迁移阶梯

### 改名范围

- 目录与模块：`apps/LotmCardStudio` → `apps/WorldOfMysteries`；`Sources/{WorldOfMysteries,WorldOfMysteriesCore,WorldOfMysteriesFeatures}`；`Tests/{WorldOfMysteriesCoreTests,WorldOfMysteriesFeaturesTests}`；入口 `WorldOfMysteriesApp.swift`；SwiftPM 包名与 product 名同为 `WorldOfMysteries`。
- `.app` 标识：`CFBundleDisplayName` 由「诡秘卡牌画册」改为「**诡秘世界**」；`CFBundleIdentifier` 由 `com.hrygo.lotm-card-studio` 改为 `com.hrygo.world-of-mysteries`；`CFBundleName`／`CFBundleExecutable` 改为 `WorldOfMysteries`；`WindowGroup` 标题同步为「诡秘世界」。
- 打包脚本：`scripts/build-app.sh` 的 `--product`、产物路径 `.build/WorldOfMysteries.app`、`Contents/MacOS/WorldOfMysteries` 共 5 处同步；`LSMinimumSystemVersion` 保持 26.0。

### 数据迁移阶梯（禁止硬切）

- 配置文件：当前路径为 `~/Library/Application Support/WorldOfMysteries/SpeechRail.json`，新增 `predecessorFileURL` 指向更名前的 `~/Library/Application Support/LotmCardStudio/SpeechRail.json` 作**只读回退**；覆盖保存只写当前路径，不写入、不删除旧文件。
- 钥匙串：服务名改为 `com.hrygo.world-of-mysteries.speechrail` 与 `...speechrail.touchid`；更名前的 `com.lotm.cardstudio.*` 保留为 `predecessor*` 只读回退。解析顺序为「当前受保护 → 当前旧式 → 更名前受保护 → 更名前旧式」，受保护条目存在但暂不可访问时立即报告不可用，不越级回退；迁移来源仅限无 userPresence 保护的旧式条目。
- 本机现状：新旧配置目录**均不存在**，本次无实际数据需要迁移；旧钥匙串条目保持「不读取、不迁移、不删除」。

### 本轮验证结果

- `swift test`：**97 passed / 0 failures**（较基线 +10：钥匙串阶梯 5 条 + 配置路径回退 4 条 + 隔离用例迁移）。
- 迁移回归用例覆盖：仅存更名前旧式条目 → `.legacy`；仅存更名前受保护条目 → `.protected`；当前名优先于更名前；当前名为空时读取更名前服务；从更名前旧式条目迁移；写入不落在任何旧服务名。
- 测试隔离：三个测试辅助构造器显式传 `fallbackFileURL: nil`，用例不依赖宿主机真实旧路径。
- `python3 tools/production.py stage-app-resources --dest <临时目录>`：`passed`，**11 张卡图 + 66 个 WAV**（此机制此前未单独验证过，本轮先行取证再进入打包）。
- `./scripts/build-app.sh release`：exit 0；`.app` 内 **CardArt 11 + Audio 66**；`lipo -archs` = **arm64**；`LSMinimumSystemVersion` = 26.0；`codesign --verify --deep --strict` = **valid on disk / satisfies its Designated Requirement**；`.app` 内无旧名产物。
- 安装与冒烟：`/Applications/WorldOfMysteries.app` 安装后启动，进程存活且无崩溃报告，随后优雅退出；旧安装 `/Applications/LotmCardStudio.app` 移入 `~/.Trash`（可回滚，未直删）。

### 仍未执行

- 2K/4K 交付像素与 ADR-003 最终采样：仍未执行，所有 `release_approved` 保持 false。
- 旧 Keychain 条目与旧配置文件的任何写入、迁移或删除：按只读回退设计**不做**。
- 历史记录回写：本文件早前小节与 `docs/research/**`、`docs/reviews/**`、`docs/superpowers/**`、`docs/decisions/ADR-004`、`docs/product/secret-world-prd-v1.0.md` 保留原文与旧路径，未回写；`generated/**` 的派生任务单由 `cardctl brief` 重新生成，未手工改写。

## 2026-09-15 二次追加：App 展示名称改为中文「诡秘世界」

### 用户裁决

- 原文：「**app 展示名称使用中文名称**」。此前 `CFBundleDisplayName` 已是「诡秘世界」，但 `CFBundleName` 为 `WorldOfMysteries`，且 bundle 目录名与显示名不一致时 Finder 回落到**文件名**，因此界面外仍显示英文名。

### 落地方式

- bundle 目录：`.build/WorldOfMysteries.app` → `.build/诡秘世界.app`（`scripts/build-app.sh` 同步，`open` 命令与 `production/AGENTS.md` 的 `stage-app-resources --dest` 示例同步）；安装路径为 `/Applications/诡秘世界.app`。
- 标识：`CFBundleName` 由 `WorldOfMysteries` 改为「诡秘世界」；`CFBundleDisplayName` 保持「诡秘世界」。
- 本地化：新增 `Resources/zh-Hans.lproj/InfoPlist.strings`（`CFBundleName`／`CFBundleDisplayName` 均为「诡秘世界」），新增 `CFBundleDevelopmentRegion = zh_CN` 与 `CFBundleLocalizations = [zh-Hans, en]`；打包脚本新增 `*.lproj` 显式拷入 `.app/Contents/Resources/`。**依据**：AppKit 从本地化资源解析 App 菜单与「强制退出」面板名称，仅改 Info.plist 键在无 `.lproj` 时不可靠。
- 不改动项：`CFBundleExecutable` 与 SwiftPM 模块名保持 ASCII `WorldOfMysteries`——进程名、日志与构建路径不引入中文；`CFBundleIdentifier` 仍为 `com.hrygo.world-of-mysteries`。

### 本轮验证结果

- `plutil -lint`：`Info.plist` 与 `zh-Hans.lproj/InfoPlist.strings` 均 OK。
- `./scripts/build-app.sh release`：exit 0；`.build/诡秘世界.app` 内 **CardArt 11 + Audio 66 + zh-Hans.lproj**，`Contents/MacOS/WorldOfMysteries`，`codesign --verify --deep --strict` = valid。
- 安装：`/Applications/诡秘世界.app` 已登记 LaunchServices；`/Applications` 下不再有英文名副本。
- 冒烟：启动后进程存活、无崩溃报告，随后优雅退出。
- 门禁：`pin_seal --check` 113 条通过；`check --level scaffold` exit 0；四个 `check-fool-*` 全部 exit 0。
- **未能验证（限制披露）**：菜单栏 App 菜单标题的 `AXTitle` 读取被系统拒绝（`osascript 不允许辅助访问`），因此「菜单栏显示中文」只有本地化资源与 `CFBundleName` 作为间接依据，仍需用户目视确认一次（另可看 ⌘-Tab 切换器与「强制退出」列表）。

### 安装与回滚方式更正（工具事实）

- 本机 `/usr/bin/trash` 实测为**直接删除**，不是移入回收站：执行后 `~/.Trash` 为空，`mdfind` 也搜不到 `LotmCardStudio.app`／`WorldOfMysteries.app`。因此旧安装与中间英文名副本**已不可从回收站恢复**——这是本轮的记录更正，后续安装步骤不再以 `trash` 充当回滚手段。
- 影响评估：两者均为可再生产物，`git show b14778f:apps/LotmCardStudio/scripts/build-app.sh` 已含 `stage-app-resources`，可在该提交上重新构建旧名 `.app`；用户数据面无损（新旧配置目录在执行时均不存在，App 自身不持有其他状态）。

### 补充：展示名称已由 LaunchServices 验证（解除上条限制披露）

- `lsappinfo find bundleid=com.hrygo.world-of-mysteries` → `ASN:0x0-0x166065f-"诡秘世界"`；`lsappinfo info -only name` → `"LSDisplayName"="诡秘世界"`，`"LSBundlePath"="/Applications/诡秘世界.app"`。
- 该登记名是 Dock、⌘-Tab 切换器与「强制退出」面板读取的系统权威来源，且**不依赖辅助功能权限**；因此「菜单栏名称为中文」不再只有间接依据。
- 残余目视项收窄为一项：菜单栏**顶部实际渲染**的 App 菜单标题仍无法用 `AXTitle` 读取（`osascript 不允许辅助访问`），但 `CFBundleName` + `zh-Hans.lproj` + `LSDisplayName` 三者已一致为「诡秘世界」。
