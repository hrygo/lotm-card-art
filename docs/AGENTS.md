...
| macOS 界面目标与交互契约（拆分 PRD） | `product/macos-ux-interaction-prd-v1.0.md` | 产品基线·拆分 PRD（`draft-for-review`）；界面与交互以它为准，引擎实现仍属另一交付面 |
| macOS 界面与交互的目标定义（拆分 PRD） | `product/macos-ux-interaction-prd-v1.0.md` | 产品基线·拆分 PRD（Agent 起草，`draft-for-review`；界面/交互以此为准，引擎实现仍属另一交付面） |
| 架构决策 | `DECISIONS.md` + `decisions/ADR-*.md` | 决策记录；逐份读 `Status`（ADR-002 仍 Proposed，ADR-003 取代其部分生产假设；ADR-006 决定不做物理迁移） |
| 分层与依赖方向 | `architecture/layering.md` | 约定·当前（全部顶层目录的层归属与依赖规则；**未机器强制**，ADR-006 派生） |
| 未来若迁移的前置条件 | `decisions/ADR-006-*.md` | 决策记录·Accepted；前置清单见其 Decision 6 |
...
- 架构·约定：`architecture/layering.md`（分层与依赖方向；未机器强制，ADR-006 派生）。
...
- 产品基线·拆分 PRD：`product/macos-ux-interaction-prd-v1.0.md`（`draft-for-review`；界面与交互目标定义，引擎实现仍属另一交付面）。
- 产品基线·拆分 PRD：`product/macos-ux-interaction-prd-v1.0.md`（《macOS UX / Interaction PRD》，Agent 起草待确认；只约束客户端界面与交互，不构成引擎立项、内容批准或视觉批准）。
