...
## D26｜客户端应用更名「卡牌演示」：显示名与 bundle 标识改挂卡牌制作工具
原因：用户裁决「卡牌制作工具的安装 app 更名为卡牌演示 demo」。本仓库已按 D24 回归纯卡牌制作职责（仓库名 `lotm-card-art`），客户端继续以《诡秘世界》产品名出现在 Finder、应用菜单与 `lsappinfo` 上就不再自洽；用户同时明确另一个仓库（`hrygo/WorldofMysteries`，`macos-app/`）里的 app **不更名**，两者从此不共用一套应用标识。
结果：
- **应用显示名**：`CFBundleDisplayName`、`CFBundleName` 与 `Resources/zh-Hans.lproj/InfoPlist.strings` 由「诡秘世界」改为「**卡牌演示**」（`.lproj` 是菜单栏名称的必要条件，理由同 D18 补充记录）；`WorldOfMysteriesApp` 的 `WindowGroup` 窗口标题同步。
- **bundle 目录与安装路径**：`.build/诡秘世界.app` → `.build/卡牌演示.app`（`scripts/build-app.sh`），安装路径 `/Applications/卡牌演示.app`。
- **bundle id**：`com.hrygo.world-of-mysteries` → `com.hrygo.lotm-card-art`，与仓库名对齐，应用标识不再挂在旧产品名下。
- **有意保持不动**：`CFBundleExecutable`、SwiftPM 包/模块名与源码目录 `apps/WorldOfMysteries` 仍是 ASCII `WorldOfMysteries`（沿用既有理由：进程名、日志、构建产物与脚本路径不引入中文；改模块名会牵动全部 `import`）；`~/Library/Application Support/WorldOfMysteries/SpeechRail.json` 与 Keychain 服务名 `com.hrygo.world-of-mysteries.speechrail*` 一律不改——它们处在既有只读回退链上，改名会让已保存的凭据失联。
- **功能零改动**：M1 画册与 M2 世界外壳、四个一级区域、示例世界、SpeechRail 设置入口全部保留；本次不裁剪、不重构界面。
- **文档同步**：根 `README.md`、根 `AGENTS.md`、`apps/AGENTS.md`、`production/AGENTS.md` 与 `apps/WorldOfMysteries/README.md` 的名称、示例路径与安装说明；机器观察记录见 [`apps/WorldOfMysteries/docs/qa/2026-09-18-app-rename.md`](../apps/WorldOfMysteries/docs/qa/2026-09-18-app-rename.md)。
- **未改口径**：`apps/`、`packages/`、`docs/product/` 的「保留面 / 归属《诡秘世界》产品」定性仍按 D24，本次只改应用名称与标识。
- **本次验证（2026-09-18）**：`swift test` 55 XCTest 用例 0 失败（与改动前基线一致；客户端 Features 目标的 Swift Testing 用例在本机 `swift test` 下报告 0 条被执行，**改动前即如此**，属既有缺口，本次未修）；`python3 -m unittest discover -s tests` **222 tests OK（skipped=6）**；`python3 tools/selfcheck.py` **7/7 步通过**；`./scripts/build-app.sh debug` 与 `release` 均退出 0，产物 `.build/卡牌演示.app` 的 `CFBundleDisplayName`/`CFBundleName` 为「卡牌演示」、`CFBundleIdentifier` 为 `com.hrygo.lotm-card-art`、`LSMinimumSystemVersion=26.0`、`lipo -archs=arm64`、`codesign --verify --deep --strict` 通过、staged 资源含 CardArt 22（11 张 × 2 档）与 Audio 66。`ditto` 安装到 `/Applications/卡牌演示.app` 后启动实测：进程存活，`lsappinfo find bundleid=com.hrygo.lotm-card-art` → `ASN:0x0-0x3c03c00-"卡牌演示"`，`LSDisplayName=卡牌演示`、`LSBundlePath=/Applications/卡牌演示.app`，随后按 bundle id 正常退出。
- **旧副本处置**：`/Applications/诡秘世界.app`（2026-09-15 release 构建）当时正在被用户运行（13:41 启动），故先不打断；用户退出该实例后，于 14:03 把它移入废纸篓 `~/.Trash/诡秘世界-20260918-1403-app-renamed-to-卡牌演示.app`（可恢复）。`/Applications` 下现在只有 `/Applications/卡牌演示.app`。
- **未完成/待决（如实记录）**：**未做用户视觉批准**（更名属可见变化，菜单栏与窗口标题的实际渲染仍需人眼确认）；显示名是否加 `Demo` 后缀未裁决；`apps/` 的「保留面 / 归属《诡秘世界》」口径未在本次裁决范围内。提交落在分支 `chore/rename-app-to-card-demo`（PR [#4](https://github.com/hrygo/lotm-card-art/pull/4)）；工作区另有在途改动（`AGENTS.md`／`README.md`／`docs/` 索引，非本次会话产生），未随该分支进入。
- **补充（2026-09-18，更名收尾）**：客户端设计 token 投影的命名随应用对齐——`config/design-tokens.json` 的 `name`、`design/design-tokens.md`、`tools/design_tokens.py` 的 CLI 描述、`design/figma-kit/main.js` 的变量集合名与面板文案，由「诡秘世界」改为「**卡牌演示**」；`python3 tools/design_tokens.py check` 仍通过（投影一致、字面量零未登记），pin 只覆盖 `production/**`，本组文件不在 pin 内，无需重封存。更名前留下的两个构建产物 `.build/诡秘世界.app` 与 `.build/WorldOfMysteries.app` 已移入废纸篓（可恢复）。**未完成**：Figma 侧集合名只在**重跑插件**时改变——重跑会新建「卡牌演示」集合，旧「诡秘世界」集合需在 Figma 内手动删除（本机不能代跑 Figma）。
