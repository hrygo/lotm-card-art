...
- **未完成/待决（如实记录）**：**未做用户视觉批准**（更名属可见变化，菜单栏与窗口标题的实际渲染仍需人眼确认）；显示名是否加 `Demo` 后缀未裁决；`apps/` 的「保留面 / 归属《诡秘世界》」口径未在本次裁决范围内；本次改动**未提交**，工作区另有在途改动（`AGENTS.md`／`README.md`／`docs/` 索引，非本次会话产生），提交边界由用户决定。
