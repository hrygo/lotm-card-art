...
- `lotm.destruction-calamity.primordial-01`：毁灭天灾，序列之上（非序列卡位，`slotID` `lotm.destruction-calamity`）。

不涉及既有的 S09「克莱恩·莫雷蒂」、S00「愚者先生」、福生玄黄天尊、上帝、堕落母神五张卡。

## 口径

- **源质＝灾祸之城**；核心象征＝灾祸 · 战争 · 毁灭；层次＝未被撕裂的旧日（第四支柱的另一半候选）。
- `sequenceName` 采用**存在名**：`序列之上 · 毁灭天灾`。[编辑裁定]
...
该卡左侧铭文槽位写的是「毁灭天灾 / THE DESTRUCTION CALAMITY」，而其余五张该槽位放的是**源质**（永暗之河／暗影世界／失序之国／知识荒野／光之钥）。本卡源质按原著为**灾祸之城**，左槽位与之不符。用户已裁决**按现状纳入**，该异常记录在 `artifacts/production/destruction-calamity-card-v1/v001/provenance.json` 的 `card_text_observed.readback_note`。叙事与语义按源质＝灾祸之城撰写，不迁就卡面错误；若日后重出图须同步更新本记录。

## 声音合同

- 使用 SpeechRail 本机 `/v1/voices` 上本轮注册的自定义 VoiceDesign 音色 `destruction-calamity`。
...
来源登记见 `artifacts/production/destruction-calamity-card-v1/v001/provenance.json`；本轮为 no-compose 整卡直出（`intermediate_2k_count = 0`），保持 `candidate-pending-user-visual-approval`。
...
- 依项目约定「**真神及以上**（序列 0 真神、天使之王、旧日/序列之上）的第三人称代词一律用 **祂**」，毁灭天灾 卡的文案已把指代该存在的人称代词统一为 **祂**（原为 `他`）。
- **发音说明（记录诚实）**：`祂` 与 `他` 在普通话中同音（`tā`），预期发音不变，因此**本轮没有重出音频、也没有逐条试听**；如要求声画严格对齐，须另行重出并试听。
- 因正文变更，`contentDigest` / `approvedDigest` / `artifacts/lotm.destruction-calamity/audio-v*/generation.json` 的 `text_digest` 与对应 Swift 叙事文件已同步重算；本轮 `check-fool-audio` 通过。
