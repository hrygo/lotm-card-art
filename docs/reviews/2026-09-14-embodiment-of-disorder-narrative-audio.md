...
- `lotm.embodiment-of-disorder.primordial-01`：失序者，序列之上（非序列卡位，`slotID` `lotm.embodiment-of-disorder`）。

不涉及既有的 S09「克莱恩·莫雷蒂」、S00「愚者先生」、福生玄黄天尊、上帝、堕落母神五张卡。

## 口径

- **源质＝失序之国**；核心象征＝秩序 · 失序 · 判断；层次＝旧日（尊名第二句为「秩序阴影」）。
- `sequenceName` 采用**存在名**：`序列之上 · 失序者`。[编辑裁定]
...
- 使用 SpeechRail 本机 `/v1/voices` 上本轮注册的自定义 VoiceDesign 音色 `embodiment-of-disorder`。
...
来源登记见 `artifacts/production/embodiment-of-disorder-card-v1/v001/provenance.json`；本轮为 no-compose 整卡直出（`intermediate_2k_count = 0`），保持 `candidate-pending-user-visual-approval`。
...
- 依项目约定「**真神及以上**（序列 0 真神、天使之王、旧日/序列之上）的第三人称代词一律用 **祂**」，失序者 卡的文案已把指代该存在的人称代词统一为 **祂**（原为 `他`）。
- **发音说明（记录诚实）**：`祂` 与 `他` 在普通话中同音（`tā`），预期发音不变，因此**本轮没有重出音频、也没有逐条试听**；如要求声画严格对齐，须另行重出并试听。
- 因正文变更，`contentDigest` / `approvedDigest` / `artifacts/lotm.embodiment-of-disorder/audio-v*/generation.json` 的 `text_digest` 与对应 Swift 叙事文件已同步重算；本轮 `check-fool-audio` 通过。
