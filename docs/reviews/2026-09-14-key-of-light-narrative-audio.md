...
- `lotm.key-of-light.primordial-01`：光之钥，序列之上（非序列卡位，`slotID` `lotm.key-of-light`）。

不涉及既有的 S09「克莱恩·莫雷蒂」、S00「愚者先生」、福生玄黄天尊、上帝、堕落母神五张卡。

## 口径

- **源质＝光之钥（与存在名同词）**；核心象征＝命运 · 概率；层次＝旧日（尊名后两句为「无尽的混乱 · 命运化身」）。
- `sequenceName` 采用**存在名**：`序列之上 · 光之钥`。[编辑裁定]
...
- 使用 SpeechRail 本机 `/v1/voices` 上本轮注册的自定义 VoiceDesign 音色 `key-of-light`。
...
来源登记见 `artifacts/production/key-of-light-card-v1/v001/provenance.json`；本轮为 no-compose 整卡直出（`intermediate_2k_count = 0`），保持 `candidate-pending-user-visual-approval`。
...
- 依项目约定「**真神及以上**（序列 0 真神、天使之王、旧日/序列之上）的第三人称代词一律用 **祂**」，光之钥 卡的文案已把指代该存在的人称代词统一为 **祂**（原为 `他`）。
- **发音说明（记录诚实）**：`祂` 与 `他` 在普通话中同音（`tā`），预期发音不变，因此**本轮没有重出音频、也没有逐条试听**；如要求声画严格对齐，须另行重出并试听。
- 因正文变更，`contentDigest` / `approvedDigest` / `artifacts/lotm.key-of-light/audio-v*/generation.json` 的 `text_digest` 与对应 Swift 叙事文件已同步重算；本轮 `check-fool-audio` 通过。
