...
`tools/selfcheck.py` 是 fresh checkout 自证入口：`scaffold → pin 漂移 → 设计 token → 愚者门禁 → 全套单测`（6 步），全通过才退 0；`--list` 只列步骤不执行，`--root` 指定仓库根。打 tag 前跑它，不替代 CI。设计与契约规则不得只靠人记得运行：新增硬规则同时接进 `selfcheck.py` 与 CI，并补反例测试。
